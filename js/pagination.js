...
(function () {
  ...
  Pagination.prototype = {
    render: function (obj) {
      ...
      this._btnNum = obj.total / obj.pagesize > parseInt(obj.total / obj.pagesize) ? parseInt(obj.total / obj
        .pagesize) + 1 : parseInt(obj.total / obj.pagesize);
    },
    bindEvent: function () {
      var that = this;
      ...
      $(that._wrapid).on('click', '#pagination-prev', function () {
        ...
        that._currentPage--;
        $('#pagination-next').hasClass('pagination-disabled') && $('#pagination-next').removeClass(
          'pagination-disabled');
      ...
      $(that._wrapid).on('click', '#pagination-next', function () {
        ...
        that._currentPage++;
        $('#pagination-prev').hasClass('pagination-disabled') && $('#pagination-prev').removeClass(
          'pagination-disabled');
      ...
      function creatBtns(ismorePosition) {
        ...
        var lastbtn = '<li class="pagenum pagination-btn" data-num=' + this._btnNum + '>' + this._btnNum +
          '</li>';
        ...
        if (this._currentPage == this._btnNum) {
          lastbtn = '<li class="pagenum pagination-btn pagination-current" data-num=' + this._btnNum + '>' +
            this._btnNum + '</li>';
        ...
        if (ismorePosition == 'none') {
          for (var i = 1; i <= this._btnNum; i++) {
            if (i == this._currentPage) {
              html += '<li class="pagenum pagination-btn pagination-current" data-num=' + i + '>' + i +
                '</li>';
            } else {
        ...
        if (ismorePosition == "all") {
          ...
          for (var i = (this._currentPage - halfnum); i <= this._currentPage + halfnum + ((this._btnsValue -
              3) % 2); i++) {
            if (i == this._currentPage) {
              html += '<li class="pagenum pagination-btn pagination-current" data-num=' + i + '>' + i +
                '</li>'
